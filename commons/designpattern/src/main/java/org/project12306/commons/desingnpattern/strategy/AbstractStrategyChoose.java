...
package org.project12306.commons.desingnpattern.strategy;

import org.project12306.commons.base.ApplicationContextHolder;
import org.project12306.commons.base.init.ApplicationInitializingEvent;
...
public class AbstractStrategyChoose implements ApplicationListener<ApplicationInitializingEvent> {
    ...
    public AbstractExecuteStrategy choose(String mark, Boolean predicateFlag) {
        if (predicateFlag != null && predicateFlag) {
            return abstractExecuteStrategyMap.values().stream()
                    .filter(each -> StringUtils.hasText(each.patternMatchMark()))//过滤是否有正则表达式实现的策略实现
     ...
     * @param mark          策略标识
     * @param requestParam  执行策略入参
     * @param predicateFlag 是否使用正则表达式解析标识
     * @param <REQUEST>     执行策略入参范型
     */
